...
|This method takes a Table with Voters in rows: each columns contains an alternative in preference order for a Voter, and the Voter is identified by the link:./GUIInputFiles.adoc[row index]. Finally, the method returns a ProfileI providing by the ProfileBuilder class.
