...
== SOC (Strict Orders - Complete List) file

SOC file allow to represent a Complete link:./profileInterfaces.adoc[Profile]. Each file with a soc extension contains a profile consisting of a complete, transitive, and asymmetric relation over a group of objects.
...
== SOI (Strict Orders - Incomplete List) file 

SOI files are derived from SOC files, but they allow to represent incomplete profiles.
...
_To indicate the indifference between two alternatives, we will use this second representation, always with oriented graphs._
